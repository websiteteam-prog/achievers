...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Fetch all students
    $sql1 = $conn->prepare("SELECT students.id as student_id, students.first_name FROM students 
                            WHERE students.grade = ? AND students.branch_id = ?");
    $sql1->bind_param("ii", $grade, $branch_id);
    $sql1->execute();
    $students_result = $sql1->get_result();

    // Fetch all dates for subject
    $sql2 = $conn->prepare("SELECT DISTINCT date FROM attendance_records 
                            WHERE subject_id = ? and MONTH(date) = MONTH(CURDATE()) 
                            AND YEAR(date) = YEAR(CURDATE()) ORDER BY date ASC");
    $sql2->bind_param("i", $subject);
    $sql2->execute();
    $dates_result = $sql2->get_result();
    $dates = [];
