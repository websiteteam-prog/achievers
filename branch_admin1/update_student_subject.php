...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    if($action === 'add') {
        $sqlAdd = $conn->prepare( "Insert into student_subjects (student_id , subject_id) values (?, ?)");
        $sqlAdd->bind_param("ii", $student_id, $subject_id);
        if ($sqlAdd->execute()){
            ...
            $sqlDelete = $conn->prepare( "delete from student_subjects where student_id = ? and subject_id = ?");
            $sqlDelete->bind_param("ii", $student_id, $subject_id);
            
            if($sqlDelete->execute()){
                echo "Subject from student removed successfully";
            } else {
